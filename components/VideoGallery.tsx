            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {videos.map((vid) => (
                    <div 
                        ...
                        className={`group bg-gray-900 border rounded-xl overflow-hidden transition-all 
                    ...
                    >
                        {vid.status === 'processing' ? (
                            ...
                            <div className="aspect-video bg-red-900/10 flex flex-col items-center justify-center p-6">
                                ...
                                <p className="text-xs text-red-500/70 mt-1">{vid.errorMessage || "Unknown error"}</p>
                            </div>
                        ) : (
                        ...
                        <div className="p-4">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                {vid.status === 'processing' && <span className="text-xs text-indigo-400 font-medium px-2 py-0.5 bg-indigo-500/10 rounded">Processing</span>}
                                {vid.status === 'failed' && <span className="text-xs text-red-400 font-medium px-2 py-0.5 bg-red-500/10 rounded">Failed</span>}
                            </div>
                        </div>
                    </div>
                ))}
