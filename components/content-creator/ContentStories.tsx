...
export const ContentStories = ({ session, onToggleSidebar }: any) => {
    ...
    return (
        <div className="flex-1 overflow-y-auto p-6 md:p-8 bg-black">
            ...
            {/* Desktop Header */}
            <h2 className="hidden md:block text-3xl font-black mb-8 text-white">Your Stories</h2>
            ...
            <div className="columns-1 md:columns-3 lg:columns-4 gap-6 space-y-6">
                {stories.map(s => {
                    ...
                    return (
                        <div key={s.id} className="break-inside-avoid bg-zinc-900 rounded-2xl overflow-hidden shadow-lg group relative border border-zinc-800">
                            {s.status === 'completed' ? (
                                <video 
                                    src={s.video_url} 
                                    controls 
                                ...
                                />
                                 ...
                                 <div className={`${aspectClass} bg-zinc-950 flex flex-col items-center justify-center p-6 text-center relative overflow-hidden`}>
                            ...
                            <div className="p-4 flex justify-between items-center bg-zinc-900">
                                <p className="text-xs text-zinc-500 font-bold">{new Date(s.created_at).toLocaleDateString()}</p>
