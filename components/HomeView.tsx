             ...
             <div className={`flex-1 flex flex-col min-w-0 relative pb-20 md:pb-0 ${mobileTab === 'settings' ? 'hidden md:flex' : 'flex'}`}>
                 <div className="hidden md:flex h-14 border-b border-gray-200 items-center justify-between px-6 bg-white flex-shrink-0 z-10">
                    <div className="flex items-center gap-4">
                        <button onClick={handleClear} className="text-gray-400 hover:text-gray-900 transition-colors p-1" title="Close Project">
                 ...
                 {showAuthModal && (
                    <div className="absolute inset-0 z-50 bg-white/90 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center">
                        ...
                        <button onClick={handleLogin} className="flex items-center gap-2 bg-black text-white px-8 py-3 rounded-full text-sm font-bold hover:bg-gray-800 transition">
                            <span>Sign in</span>
                        </button>
                    </div>
                 )}
             ...
             <div className={`w-full md:w-80 md:border-l border-gray-200 bg-white flex flex-col z-10 flex-shrink-0 pb-20 md:pb-0 ${mobileTab === 'preview' ? 'hidden md:flex' : 'flex h-full'}`}>
                 <div className="h-14 border-b border-gray-200 flex items-center px-6 flex-shrink-0 bg-gray-50/50">
                     ...
                     <div className="ml-auto">
                         {profile && <span className={`text-[10px] font-mono border px-2 py-1 rounded font-bold ${profile.credits > 0 ? 'text-gray-500 border-gray-200 bg-white' : 'text-red-500 border-red-200 bg-red-50'}`}>{profile.credits} CREDITS</span>}
                     </div>
                 </div>
                 ...
                 <div className="flex-1 overflow-y-auto p-6 space-y-8">
                         ...
                         <div className="space-y-1">
                             {VOICES.map(v => (
                                 <button
                                    key={v.id}
                                    onClick={() => setVoice(v)}
                                    className={`w-full flex items-center justify-between px-3 py-3 md:py-2.5 text-sm border rounded-lg transition-all duration-200 ${voice.id === v.id ? 'bg-green-50 text-green-800 border-green-500 shadow-sm ring-1 ring-green-500' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300 hover:bg-gray-50'}`}
                                 >
                     ...
                     {voice.id !== 'voiceless' && (
                        <>
                            <div className="space-y-3">
                                <label className="text-xs font-bold text-gray-900 uppercase tracking-wider flex justify-between">App Name<span className="text-green-600">*</span></label>
                                <input type="text" className="w-full bg-white border border-gray-300 p-3 text-sm text-gray-900 focus:border-green-500 focus:ring-1 focus:ring-green-500 focus:outline-none transition-all rounded-md placeholder-gray-400 font-medium" placeholder="e.g. TrustMRR" value={appName} onChange={(e) => setAppName(e.target.value)} />
                            </div>
                            <div className="space-y-3">
                                <label className="text-xs font-bold text-gray-900 uppercase tracking-wider flex justify-between">App Description<span className="text-green-600">*</span></label>
                                <textarea className="w-full bg-white border border-gray-300 p-3 text-sm text-gray-900 focus:border-green-500 focus:ring-1 focus:ring-green-500 focus:outline-none transition-all min-h-[140px] resize-none placeholder-gray-400 leading-relaxed rounded-md font-medium" placeholder="Supabase is a Postgres..." value={appDescription} onChange={(e) => setAppDescription(e.target.value)} />
                            </div>
                        </>
                     )}

                     {errorMessage && <div className="text-xs text-red-600 bg-red-50 border border-red-200 p-3 rounded font-medium">{errorMessage}</div>}
                 </div>

                 <div className="p-6 border-t border-gray-200 flex-shrink-0 bg-gray-50 pb-safe">
                     <button onClick={triggerGenerate} disabled={!session || !isDurationValid} className="w-full py-4 bg-green-600 text-white text-base font-bold rounded-xl hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-green-200 transform active:scale-[0.98]">Generate Demo</button>
                    {profile && profile.credits < 1 && <button onClick={onPurchase} className="block w-full mt-4 text-center text-xs font-bold text-gray-500 hover:text-green-600 transition-colors">Purchase Credits</button>}
                 </div>
             </div>
